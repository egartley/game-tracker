...
if (!isset($_POST["passinput"])) {
    echo '<!DOCTYPE html><html lang="en"><head><title>Editor Login</title>';

    require "../includes/header.php";
    get_header();
    get_stylesheet("input.css");

    echo '</head><body><div class="content"><div class="page-title">
        <span>Editor Login</span></div><div class="input-outer-container">';

    if (isset($_GET["wrong"])) {
        echo '<div class="input-container"><p>Wrong password, try again!</p></div>';
    }

    echo '<div class="input-container"><form action="/login/index.php" method="post">
        <label for="passinput">Password:</label><input id="passinput" name="passinput" type="password">';
